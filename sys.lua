...
function board_info()
    ...
    local sys_info = nixio.sysinfo()
    local model = "unknown"
    local release = "unknown"

    -- Try to read model from /tmp/sysinfo/model
    local model_file = fs.readfile("/tmp/sysinfo/model")
    if model_file then
        model = model_file:match("^(.*)$")
    end

    -- Try to read release info from /etc/openwrt_release
    local release_file = fs.readfile("/etc/openwrt_release")
    if release_file then
        local distrib_id = release_file:match('DISTRIB_ID=[\'"](.-)[\'"]')
        local distrib_release = release_file:match('DISTRIB_RELEASE=[\'"](.-)[\'"]')
        if distrib_id and distrib_release then
    ...
    return {
        ...
        memory   = {
            total = sys_info.totalram,
            free  = sys_info.freeram
        },
...
function net.ipaddrs(callback)
	local ifn = { }
	for _, e in ipairs(nixio.getifaddrs()) do
		if e.name ~= "lo" then
			ifn[e.name] = ifn[e.name] or { }
			if e.family == "packet" and e.addr and #e.addr == 17 then
				ifn[e.name].mac = e.addr:upper()
			elseif e.family == "inet" then
				ifn[e.name].ipv4 = e.addr
			elseif e.family == "inet6" then
				ifn[e.name].ipv6 = e.addr
			end
		end
	end

	if callback then
		for name, data in pairs(ifn) do
			callback(name, data)
		end
	else
		return ifn
...
function led.set_trigger(name, trigger)
    local uci_cursor = uci.cursor()
    local found = false

    uci_cursor:foreach("system", "led", function(s)
        if s.name == name then
            uci_cursor:set("system", s[".name"], "trigger", trigger)
            found = true
        end
    end)

    if not found then
        return { success = false, error = "LED not found: " .. name }
    end

    uci_cursor:commit("system")
    os.execute("/etc/init.d/led restart >/dev/null 2>&1")

    return { success = true, message = "LED " .. name .. " trigger set to " .. trigger }
end
